...
*Notes: This is a markdown version of projects. This file is the same as the file called "Course 2 Waze Project", and standard answer key was also uploaded*
...
As we can see, there are 700 missing value in columns "label".

In real world scenario, we have a few different option:
1. Delete the data that contain missing value. This works when missing data only have a small proportion of whole data set, are not affect the data pattern.
2. Fill value with "unknown" or similar options. 

If we have too many missing value, we need to consider re-collect data. 

If me, I would choose method #1; But this projects has chose analysis both missing value and complete value seperately.
...
### YOUR CODE HERE ###
data_null = data[data["label"].isnull()]
...
### YOUR CODE HERE ###
data_non_null = data[~data["label"].isnull()]
# Display summary stats of rows without null values
### YOUR CODE HERE ###
data_non_null.describe()
```
...
After check both table, we can't see a difference, and there are not missing value, too.
...
447 iPhone users and 253 Android users.
...
### YOUR CODE HERE ###
data_null["device"].value_counts(normalize = True)
```
...
### YOUR CODE HERE ###
data.groupby("label").median(numeric_only= True)
```
...
In here, I want to add a mean, just for reference: 
...
### YOUR CODE HERE ###
medians_by_label = data.groupby("label").median(numeric_only=True)
...
1. It has 700 missing values in lable columns. There is no obvious parrten for missing data.
2. Mean value might be affacted by extreme value where median value will only focus around middle value.
3. It seems like there are some unusual data in our result. For example, median drivers driving day, churned user are driver much more than retained user? Should't it be opposite?
4. iPhone user is about 64% and Android user is about 35%.
5. Churning users have a higher median number of drives per driving day, which is indeed interesting and may seem counterintuitive at first glance. Based on that, I can't tell from this analysis.
6. No. There are no major difference, almost the same.
